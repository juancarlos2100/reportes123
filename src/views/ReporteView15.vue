            ...
            <div class="left-container" style="flex: 0 0 90%; overflow: auto;">
                <div v-for="(grupo, producto) in agrupadosPorProducto" :key="producto">
                    ...
                    <table :class="{ 'table': !isDarkMode, 'dark-mode-table': isDarkMode }">
                        ...
                        <tbody>
                            <tr v-for="(compra, index) in grupo" :key="index">
                                <td><strong>{{ compra.proveedor }}</strong></td>
                                <td>{{ compra.referencia }}</td>
                                <td>{{ compra.fechaRegistro }}</td>
                                <td>{{ compra.fechaCompra }}</td>
                                <td>{{ compra.producto }}</td>
                                <td>{{ parseFloat(compra.cantidad).toLocaleString('en-US', {minimumFractionDigits: 2, maximumFractionDigits: 2}) }}L</td>
                                <td>{{ parseFloat(compra.precio).toLocaleString('en-US', {minimumFractionDigits: 2, maximumFractionDigits: 2}) }}</td>
                                <td>${{ parseFloat(compra.total).toLocaleString('en-US', {minimumFractionDigits: 2, maximumFractionDigits: 2}) }}</td>
                            </tr>
                            <tr>
                                <td style="font-size: larger;" colspan="6"><tr><strong>Total</strong></tr></td>
                                <td style="font-size: larger;"><strong>{{ calcularTotal(grupo, 'cantidad').toLocaleString('en-US', {minimumFractionDigits: 2, maximumFractionDigits: 2}) }}L</strong></td>
                ...
                <h2 style="text-align: right; margin-right: 120px;">Total General</h2>
                    <table :class="{ 'table': !isDarkMode, 'dark-mode-table': isDarkMode } " style="margin-left: 1620px; width: 27%">
                        ...
                        <tr>
                            <th colspan="6"></th>
                            <th style="text-align: right;">Total Cantidad</th>
                        ...
                        <tr>
                            <td colspan="6"></td>
                            <td style="text-align: right; font-size: larger;"><strong>{{ calcularTotalGeneral('cantidad').toLocaleString('en-US', {minimumFractionDigits: 2, maximumFractionDigits: 2}) }}L</strong></td>
                    ...
                    <div class="container" style="display: flex; justify-content: space-around;">
                            <div class="chart-container" style="width: 60%; margin-left: 100px;">
                                <canvas id="pieChartCantidad" style="width: 100%; height: 600px;"></canvas>
                            </div>
...
export default {
    data() {
        return {
            isDarkMode: true,
            resultados: [],  // Asegúrate de inicializar esto como un arreglo vacío
    ...
    const agrupados = {};
    if (!this.resultados) return agrupados; // Verificación adicional para evitar el error
            this.resultados.forEach((compra) => {
                compra.compras.forEach((detalle) => {
                    if (!agrupados[detalle.producto]) {
                        agrupados[detalle.producto] = [];
                    }
                    agrupados[detalle.producto].push({
    ...
    methods: {
        ...
        calcularTotalGeneral(campo) {
            return Object.values(this.agrupadosPorProducto).reduce((total, grupo) => total + this.calcularTotal(grupo, campo), 0);
        },
